...
from .data import SubDataset, Dataset, TxAdaptingDataset, TxAdaptingSubDataset
from .model import MLP, ComPert, TxAdaptingComPert, _move_inputs
...
def compute_prediction(autoencoder: TxAdaptingComPert, genes, emb_drugs, emb_covs):
    """
...
def evaluate_logfold_r2(
    autoencoder: TxAdaptingComPert, ds_treated: TxAdaptingSubDataset, ds_ctrl: TxAdaptingSubDataset
):
    logfold_score = []
    signs_score = []
    ...
    for cell_drug_dose_comb, category_count in zip(
        *np.unique(ds_treated.pert_categories, return_counts=True)
        ...
        # )
        bool_de = np.ones(ds_treated.var_names.shape[0]).astype(bool)
...
def evaluate_disentanglement(autoencoder: TxAdaptingComPert, data: TxAdaptingDataset, device="cpu"):
    """
    ...
    with torch.no_grad():
        if data.use_drugs_idx:
            genes, drugs_idx, dosages, covariates = _move_inputs(
                data.genes, data.drugs_idx, data.dosages, data.covariates, device=device
            )
    ...
    if data.perturbation_key is not None:
        ...
        pert_score = compute_score(data.smiles)
...
def evaluate_r2_tx_adapting(autoencoder: TxAdaptingComPert, dataset: TxAdaptingSubDataset, genes: torch.Tensor, device="cpu"):
    emb_covs = dataset.covariates
    drugs_idx = dataset.drugs_idx
    dosages = dataset.dosages

    genes, drugs_idx, dosages, emb_covs = _move_inputs(genes, drugs_idx, dosages, emb_covs, device=device)
    emb_drugs = (
        drugs_idx,
        dosages,
    )
    ...
    y_true = genes.detach().cpu()
    mean_pred = torch.clamp(mean_pred, -3e12, 3e12)
    y_pred = mean_pred.detach().cpu()
    metric = R2Score(num_outputs=y_pred.shape[1], multioutput='uniform_average')
    metric.update(y_pred, y_true)  # same as sklearn.r2_score(y_true, y_pred)
    return metric.compute().item()


def evaluate_r2(autoencoder: TxAdaptingComPert, dataset: TxAdaptingSubDataset, genes_control: torch.Tensor):
    """
    ...
    for cell_drug_dose_comb, category_count in zip(
        *np.unique(dataset.pert_categories, return_counts=True)
        ...
        mean_score_de.append(0.0)
        var_score_de.append(0.0)
    if len(mean_score) > 0:
        return [
...
def evaluate_tx_adapting(
    autoencoder: TxAdaptingComPert,
    datasets: Dict[str, TxAdaptingDataset],
    eval_stats,
    ...
    if run_disentangle:
        ...
        smiles, drug_counts = np.unique(
            datasets["test"].smiles, return_counts=True
        )
        disent_scores = evaluate_disentanglement(autoencoder, datasets["test"], device=device)
    ...
    with torch.no_grad():
        ...
        if run_r2:  # NOTE: Predictions are made in the AE manner instead of the original ChemCPA counterfactual manner
            logging.info("Running R2 evaluation")
            evaluation_stats["training"] = evaluate_r2_tx_adapting(
                autoencoder,
                datasets["training"],
                datasets["training"].genes,
                device=device,
            )
            if "test" in eval_stats:
                evaluation_stats["test"] = eval_stats["test"]
            else:
                evaluation_stats["test"] = evaluate_r2_tx_adapting(
                    autoencoder,
                    datasets["test"],
                    datasets["test"].genes,
                    device=device,
                )
            if "val" in eval_stats:
                evaluation_stats["val"] = eval_stats["val"]
            else:
                evaluation_stats["val"] = evaluate_r2_tx_adapting(
                    autoencoder,
                    datasets["val"],
                    datasets["val"].genes,
                    device=device,
        ...
        if run_logfold:
            ...
            evaluation_stats["test"] = evaluate_logfold_r2(
                autoencoder,
                datasets["test"],
                datasets["test"],
            )
