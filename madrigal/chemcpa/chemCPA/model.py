...
def to_device(input, device):
    ...
    elif isinstance(input, torch.Tensor):  # NOTE: don't put some random stuff in the batch
...
class TxAdaptingComPert(torch.nn.Module):
    """
    ...
    def __init__(
        ...
        num_genes: int,
        num_drugs: int,
        covariate_names_unique: Dict[str, List[str]],
        seed=0,
        patience=5,
        doser_type="logsigm",
        ...
        **kwargs,
        ...
        super(TxAdaptingComPert, self).__init__()
        # set generic attributes
        self.num_genes = num_genes
        self.num_drugs = num_drugs
        self.covariate_names_unique = covariate_names_unique
        self.num_covariates = [
        ...
        self.init_args = {
            ...
            "covariate_names_unique": self.covariate_names_unique,
            "seed": self.seed,
        ...
        if use_drugs:
            ...
            if doser_type == "mlp":
                ...
                self.dosers = GeneralizedSigmoid(
                    self.num_drugs, "cpu", nonlin=doser_type
                )
            self.doser_type = doser_type
        
        else:
            self.adversary_drugs = self.drug_embeddings = self.drug_embedding_encoder = self.dosers = None
            embedding_requires_grad = False

        assert 0 not in self.num_covariates
        self.adversary_covariates = []
        self.loss_adversary_covariates = []
        self.covariates_embeddings: torch.nn.ModuleList[torch.nn.Embedding] = torch.nn.ModuleList(
            []
        ...
        _parameters = (
            ...
            + get_params(self.drug_embeddings, has_drugs and embedding_requires_grad and use_drugs)
            + get_params(self.drug_embedding_encoder, use_drugs)
        )
        ...
        _parameters = get_params(self.adversary_drugs, has_drugs and use_drugs and not disable_adv)
        for adv in self.adversary_covariates:
            _parameters.extend(get_params(adv, has_covariates and not disable_adv))
    ...
    def set_hparams_(self, seed, hparams):
        ...
        np.random.seed(seed)
        self.hparams = {
            "dim": 128 if default else int(np.random.choice([128, 256, 512])),
            ...
            "batch_size": 8192
            if default
            else int(np.random.choice([4096, 8192, 16384])),
    ...
    def predict(
        ...
        if self.num_drugs > 0 and self.use_drugs:
        ...
        if self.num_covariates[0] > 0:
            for cov_type, emb_cov in enumerate(self.covariates_embeddings):
                emb_cov = emb_cov.to(latent_treated.device)
                cov_idx = covariates[cov_type].argmax(1)
                latent_treated = latent_treated + emb_cov(cov_idx)

        cell_drug_embedding = torch.cat([emb_cov(cov_idx), drug_embedding], dim=1) if drug_embedding is not None else emb_cov(cov_idx)
        ...
        output = (normalized_reconstructions, cell_drug_embedding)
        if return_latent_basal:
            output += (latent_basal, )
        if return_latent_treated:
            output += (latent_treated, )
    ...
    def update(
        ...
        assert (drugs is not None) or (drugs_idx is not None and dosages is not None)
        genes, drugs, drugs_idx, dosages, covariates = _move_inputs(
            genes, drugs, drugs_idx, dosages, covariates, device=device
        )
        ...
        adversary_drugs_loss = torch.tensor([0.0], device=device)
        if self.num_drugs > 0 and self.use_drugs and not self.disable_adv:
        ...
        adversary_covariates_loss = torch.tensor([0.0], device=device)
        if self.num_covariates[0] > 0 and not self.disable_adv:
        ...
        adv_drugs_grad_penalty = torch.tensor([0.0], device=device)
        adv_covs_grad_penalty = torch.tensor([0.0], device=device)

        if (self.iteration % self.hparams["adversary_steps"]) == 0 and not self.disable_adv:
