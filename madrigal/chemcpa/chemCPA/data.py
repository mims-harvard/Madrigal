...
from .helper import canonicalize_smiles
...
class TxAdaptingDataset:
    covariate_keys: Optional[List[str]]
    drugs_idx: torch.Tensor  # stores the integer index of the drugs applied to each cell.
    dosages: torch.Tensor  # shape: (dataset_size, max_num_perturbations)
    smiles_unique_sorted: np.ndarray  # sorted list of all smiles in the dataset
    
    def __init__(
        self,
        dataset_path: str,
        perturbation_key="canonical_smiles",
        dose_key="pert_dose",
        covariate_keys="cell_iname",
        smiles_key="canonical_smiles",
        pert_category="max_dose_averaged_sig_id",
        split_key="split",
        **kwargs,
    ):
        """
        ...
        :param pert_category: Name of obs column with stores covariate + perturbation + dose as one string.
            Example: cell type + drug name + drug dose. This is used during evaluation. 
            NOTE: We've made sure this is unique for each row.
        """
        logging.info(f"Starting to read in data from {dataset_path}\n...")
        metadata: pd.DataFrame = pd.read_pickle(dataset_path + 'tx_cp_metadata_for_adapting.pkl')
        data: pd.DataFrame = pd.read_csv(dataset_path + 'tx_cp_data_averaged_for_adapting.csv', index_col=0)
        assert np.all(data.columns.values == metadata['max_dose_averaged_sig_id'].values)
        logging.info(f"Finished data loading.")
        
        self.genes = torch.Tensor(data.values.T)  # this is actually not genes but compound_sigs.  keeping the naming to avoid conflict.
        self.var_names = data.index.values

        self.perturbation_key = perturbation_key
        self.dose_key = dose_key
        covariate_keys = [covariate_keys] if isinstance(covariate_keys, str) else covariate_keys
        self.covariate_keys = covariate_keys
        self.smiles_key = smiles_key
        self.use_drugs_idx = True

        assert perturbation_key is not None
        assert dose_key is not None
        self.pert_categories = metadata[pert_category].values
        self.smiles = metadata[perturbation_key].values
        self.dosages = metadata[dose_key].values

        # get unique drug smiles
        assert np.all(np.sort(metadata[perturbation_key].unique()) == metadata[perturbation_key].unique())
        self.smiles_unique_sorted = metadata[perturbation_key].unique()

        self._smiles_to_idx = {
            smiles: idx for idx, smiles in enumerate(self.smiles_unique_sorted)
        }
        self.canon_smiles_unique_sorted = self.smiles_unique_sorted
        
        drugs_idx = [self.smiles_to_idx(drug) for drug in self.smiles]
        self.drugs_idx = torch.LongTensor(drugs_idx)
        dosages = self.dosages.astype(float)
        self.dosages = torch.from_numpy(dosages).float()

        assert len(covariate_keys) == len(set(covariate_keys)), f"Duplicate keys were given in: {covariate_keys}"
        self.covariate_names = {}
        self.covariate_names_unique = {}
        self.atomic_сovars_dict = {}
        self.covariates = []
        for cov in covariate_keys:
            self.covariate_names[cov] = metadata[cov].values
            self.covariate_names_unique[cov] = np.unique(self.covariate_names[cov])
            ...
            names = self.covariate_names[cov]
            self.covariates.append(
                torch.from_numpy(encoder_cov.transform(names.reshape(-1, 1))).float()
            )
        ...
        self.num_genes = self.genes.shape[1]
        self.num_drugs = len(self.smiles_unique_sorted)

        self.indices = {
            "all": list(range(len(self.genes))),
            # "treated": list(range(len(self.genes))),
            "train": np.where(metadata[split_key].values == "train")[0].tolist(),
            "val": np.where(metadata[split_key].values == "val")[0].tolist(),
            "test": np.where(metadata[split_key].values == "test")[0].tolist(),
        }

    def subset(self, split):
        return TxAdaptingSubDataset(self, self.indices[split])

    def smiles_to_idx(self, smiles: str):
        """
        For the given drug, return it's index. The index will be persistent for each dataset (since the list is sorted).
        Raises ValueError if the drug doesn't exist in the dataset.
        """
...
class TxAdaptingSubDataset:
    def __init__(self, dataset: TxAdaptingDataset, indices):
        ...
        self.covariate_names_unique = dataset.covariate_names_unique
        self.covariates = [indx(cov, indices) for cov in dataset.covariates]

        self.smiles = indx(dataset.smiles, indices)
        self.pert_categories = indx(dataset.pert_categories, indices)
        self.covariate_names = {}
        assert (
            "cell_iname" in self.covars_dict
        ), "`cell_iname` must be provided as a covariate"
        ...
        self.num_genes = dataset.num_genes  # TODO: Check this
...
def load_dataset_splits_tx_adapting(
    dataset_path: str,
    perturbation_key: Union[str, None] = "canonical_smiles",
    dose_key: Union[str, None] = "pert_dose",
    covariate_keys: Union[list, str, None] = "cell_iname",
    smiles_key: Union[str, None] = "canonical_smiles",
    pert_category: str = "max_dose_averaged_sig_id",
    split_key: str = "split",
    return_dataset: bool = False,
    **kwargs
):
