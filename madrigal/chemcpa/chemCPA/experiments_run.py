...
from .data import load_dataset_splits, load_dataset_splits_tx_adapting
from .embedding import get_chemical_representation
from .model import ComPert, TxAdaptingComPert
from .profiling import Profiler
from .train import custom_collate, evaluate, evaluate_r2, evaluate_tx_adapting, evaluate_r2_tx_adapting
...
class TxAdaptingExperimentWrapper:
    """
    ...
    def init_dataset(self, data_params: dict, **kwargs):
        """
        ...
        self.datasets, self.dataset = load_dataset_splits_tx_adapting(
    ...
    def init_drug_embedding(self, embedding: dict):
        ...
        if not embedding["directory"].startswith(DATA_DIR):
            embedding["directory"] = DATA_DIR + embedding["directory"]
        self.drug_embeddings = get_chemical_representation(  # ComPert will use the provided embedding, which is frozen during training
    ...
    def init_model(
        ...
        if load_pretrained:
            (
                ...
                pretrained_model_ckpt, pretrained_model_path, append_ae_layer
            )
            append_layer_width = (
            ...
            self.autoencoder = TxAdaptingComPert(
                in_out_size,
                self.datasets["training"].num_drugs,
                model_config["covariate_names_unique"],
                **additional_params,
                hparams=hparams,
            ...
            self.autoencoder = TxAdaptingComPert(
                ...
                self.datasets["training"].num_drugs,
                self.datasets["training"].covariate_names_unique,
                **additional_params,
                hparams=hparams,
                drug_embeddings=self.drug_embeddings,
    ...
    def load_state_dict(
        self, pretrained_model_ckpt, pretrained_model_path, append_ae_layer
    ):
        filename = pretrained_model_ckpt + ".pt"
    ...
    def train(
        ...
        if not save_dir.startswith(BASE_DIR):
        ...
        for epoch in range(num_epochs):
            ...
            if self.autoencoder.num_drugs > 0 and self.autoencoder.use_drugs:
            ...
            if ((epoch % checkpoint_freq) == 0 and epoch > 0) or stop:
                ...
                #     else None
                # )
                test_score = evaluation_stats["test"] if evaluation_stats["test"] else None
                ...
                if (
                    ...
                    evaluation_stats = evaluate_tx_adapting(
                        self.autoencoder,
                        self.datasets,
                ...
                if save_checkpoints and stop:
                    ...
                    file_name = f"{cur_time}_{run_name}.pt"
                    torch.save(
                        (
...
def get_experiment(init_all=False):
    ...
    experiment = TxAdaptingExperimentWrapper(init_all=init_all)
    return experiment
...
def train(experiment=None):
    if experiment is None:
        experiment = TxAdaptingExperimentWrapper()
