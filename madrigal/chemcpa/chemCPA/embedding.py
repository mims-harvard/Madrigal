...
from .paths import VIEW_OUTPUT_DIR


def get_chemical_representation(
    smiles: List[str], 
    embedding_model: str, 
    embedding_dir: str = VIEW_OUTPUT_DIR + 'tx/embeddings/',
    device: str = 'cuda'
):
    assert embedding_model == 'rdkit'
    df = pd.read_parquet(embedding_dir + "rdkit2D_embeddings_all_all_normalized.parquet")
