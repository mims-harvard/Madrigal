...
class APIRequest:
    ...
    async def call_api(
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    url=request_url, headers=request_header, json=self.request_json['request']
                ) as response:
        ...
        if error:
            ...
            # TODO: here is where we may want to also store the template id and text along with the request and reponse
...
def num_tokens_consumed_from_request(
    ...
    if api_endpoint.endswith("completions"):
        max_tokens = request_json['request'].get("max_tokens", 15)
        n = request_json['request'].get("n", 1)
        completion_tokens = n * max_tokens

        # chat completions
        if api_endpoint.startswith("chat/"):
            num_tokens = 0
            for message in request_json['request']["messages"]:
        ...
        # normal completions
            ...
            prompt = request_json['request']["prompt"]
    ...
    # if embeddings request, tokens = input tokens
        ...
        input = request_json['request']["input"]
